...
*** Installation
uv handles Python and all Python dependencies automatically. The only things that need to be installed separately are system libraries, which require [[https://brew.sh][Homebrew]] on macOS:
#+begin_src shell
